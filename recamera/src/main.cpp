        ...
        if (camera->retrieveFrame(frame, MA_PIXEL_FORMAT_RGB888) == MA_OK) {
            ...
            if (model->getOutputType() == MA_OUTPUT_TYPE_BBOX) {
                ...
                for (auto result : _results) {
                    ...
                    printf("- Class %d: %.2f confidence at [%.0f,%.0f,%.0f,%.0f], Approx Distance: %.2f meters\n", 
                           result.target, result.score, 
                           result.x * input_width, result.y * input_height, 
                           (result.x + result.w) * input_width, (result.y + result.h) * input_height,
                           distance);
