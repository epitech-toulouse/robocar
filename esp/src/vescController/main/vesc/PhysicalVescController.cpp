...
void PhysicalVescController::activate() {
    this->active = true;
}

void PhysicalVescController::deactivate() {
    this->active = false;
    this->set_speed(0.0f);
}

bool PhysicalVescController::isActive() {
    return this->active;
}
...
void PhysicalVescController::stop() {
    set_speed(0.0f);
}

void PhysicalVescController::set_steering(float steering) {
    if (this->active) {
        if (steering < 0.0f)
            steering = 0.0f;
        if (steering > 1.0f)
...
void PhysicalVescController::sendPacket(const uint8_t* payload, int len) {
    uint8_t frame[256];
    int idx = 0;
...
uint16_t PhysicalVescController::crc16(const uint8_t* buf, int len) {
    uint16_t crc = 0;
    for (int i = 0; i < len; i++) {
