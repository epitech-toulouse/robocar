...
esp_err_t rmt_uart_init(rmt_uart_port_t uart_num, const rmt_uart_config_t* uart_config)
{
    ...
    rmt_uart_contex[uart_num].rmt_uart_contex_tx.items = NULL;
...
#endif
        ESP_RETURN_ON_FALSE((rmt_uart_contex[uart_num].rmt_uart_contex_tx.items), ESP_ERR_NO_MEM, TAG, "tx items alloc failed");
    }
...
esp_err_t rmt_uart_write_bytes(rmt_uart_port_t uart_num, const uint8_t* data, size_t size)
{
    ...
    ESP_RETURN_ON_FALSE((size > 0), ESP_ERR_INVALID_ARG, TAG, "size is zero");
    rmt_uart_contex_tx_t* rtc = &ctx->rmt_uart_contex_tx;
    ESP_RETURN_ON_FALSE((rtc->items != NULL), ESP_FAIL, TAG, "tx items not allocated");
